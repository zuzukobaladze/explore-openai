# This script implements a Streamlit chatbot application capable of handling text-based, image-based, and now audio-based queries.
...
def ask_openai(user_question, temperature=1.0, top_p=1.0, max_tokens=1000):
...
# Updated function to reflect the actual response contract:
# { "text": "Imagine the wildest idea..." }


def ask_openai_audio(audio_bytes, user_prompt=None):
    import io

    # Create an in-memory file-like object for the audio
    file_obj = io.BytesIO(audio_bytes)
    file_obj.name = "audio.wav"  # Provide a filename with extension

    # Call the new transcriptions API via our OpenAI client
    transcript = client.audio.transcriptions.create(
        model="whisper-1", file=file_obj, prompt=user_prompt or ""
    )

    # The returned object has the shape: {"text": "..."}
...
def display_chat():
    ...
    for msg in st.session_state.current_chat:
        if msg.sender == USER:
            ...
            # Check if the user message includes an inline base64 audio
            elif "data:audio/wav;base64," in msg.content:
                splitted = msg.content.split("data:audio/wav;base64,")
                text_part = splitted[0].strip()
                base64_part = splitted[1] if len(splitted) > 1 else None
                # If there is text before the audio, we display it
                if text_part:
                    st.chat_message("human").write(text_part)
                # Then we decode and display the audio, if present
                if base64_part:
                    try:
                        audio_bytes = base64.b64decode(base64_part.strip())
                        st.chat_message("human").audio(audio_bytes)
                    except Exception as e:
                        logger.error(f"Error decoding base64 audio: {e}")
                        st.chat_message("human").write("[Error decoding audio]")
            else:
                # If no image or audio is included, display text only
...
# This function handles user submissions from the form, either text, image + text, or audio + text


def handle_submission(prompt: str, image_file, audio_file):
    ...
    # If an audio file was uploaded (and no image)
    elif audio_file:
        audio_bytes = audio_file.read()
        b64 = base64.b64encode(audio_bytes).decode("utf-8")
        # Combine prompt text and the audio data as a single user message
        combined = prompt + ("\n" if prompt else "") + f"data:audio/wav;base64,{b64}"

        # Display user's text & audio
        if prompt.strip():
            st.chat_message("user").write(prompt)
        ...
        st.session_state.current_chat.append(
            ChatMessage(sender=USER, content=f"data:audio/wav;base64,{b64}")
        )

        # Update the last message preview to reflect text + audio
        if prompt.strip():
            last_msg = f"{prompt} [AUDIO]"
        else:
            last_msg = "[AUDIO]"

        # Now use a spinner while transcribing the audio
        with st.spinner("Transcribing audio..."):
            ai_content = ask_openai_audio(audio_bytes, prompt)

        # Display the transcription as the AI's response
        with st.chat_message("ai"):
            st.write(ai_content)
...
def run():
    ...
    with form_container:
        with st.form("input_form", clear_on_submit=True):
            ...
            audio_file = st.file_uploader(
                "Upload an audio", type=["wav", "mp3", "m4a", "ogg"]
            )
    ...
    with chat_container:
        if submitted:
            handle_submission(prompt, image_file, audio_file)
        else:
            display_chat()
...
# ChatGPT Link
# https://chatgpt.com/share/6796572b-bbf8-8010-89de-e615bd84dd14
