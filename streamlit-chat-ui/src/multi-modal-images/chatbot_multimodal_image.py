...
def run():
    ...
    if st.session_state.chat_history and st.session_state.active_chat_id:
        ...
        if prompt:  # Check if text input is provided
            st.chat_message("user").write(prompt)  # Display the user's text input
            st.session_state.current_chat.append(
                ChatMessage(content=prompt, sender=USER)  # Save the user's text message
            )
            ...
            with st.chat_message("ai"):
                ...
                st.write(ai_message)  # Display the AI's response
        ...
        if uploaded_file:
            ...
            st.image(file_path, caption="Uploaded image")  # Display the uploaded image
            st.session_state.current_chat.append(
                ChatMessage(image_path=file_path, sender=USER)  # Save the image message
            )
